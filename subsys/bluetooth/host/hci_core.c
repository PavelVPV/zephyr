 ...
 */
static inline uint8_t bt_hci_evt_get_flags(uint8_t evt, uint8_t subevent)
{
	switch (evt) {
...
static void hci_event(struct net_buf *buf)
{
	...
	if (hdr->evt == BT_HCI_EVT_LE_META_EVENT) {
		...
		subevent = le_evt->subevent;
	}

	BT_ASSERT(bt_hci_evt_get_flags(hdr->evt, subevent) & BT_HCI_EVT_FLAG_RECV);
...
void hci_event_prio(struct net_buf *buf)
{
	...
	evt_flags = bt_hci_evt_get_flags(hdr->evt, subevent);
...
static int bt_recv_unsafe(struct net_buf *buf)
{
	...
	case BT_HCI_H4_EVT:
	{
		...
		uint8_t evt_flags = bt_hci_evt_get_flags(hdr->evt, subevent);
