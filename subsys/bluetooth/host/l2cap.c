...
{
	/* This function accesses l2cap_data_ready list and is the only one that can be called from
	 * a preemptive thread context, therefore requires a critical section to ensure that the
	 * data ready list is not modified while we are checking and appending to it.
	 */
	k_sched_lock();
	...
	if (!sys_slist_find(&le_chan->chan.conn->l2cap_data_ready,
		...
		sys_slist_append(&le_chan->chan.conn->l2cap_data_ready,
				 &le_chan->_pdu_ready);
		LOG_DBG("L2CAP channel %p added to data ready list", le_chan);
	} else {
		LOG_DBG("L2CAP channel %p already in data ready list", le_chan);
	}
