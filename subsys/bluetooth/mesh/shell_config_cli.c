...
int cmd_timeout(const struct shell *sh, size_t argc, char *argv[])
{
	int32_t timeout_ms;
	int err = 0;
...
static int cmd_get_comp(const struct shell *sh, size_t argc, char *argv[])
{
	...
	shell_print(sh, "Got Composition Data for 0x%04x:", net.dst);
...
static int cmd_polltimeout_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_lpn_timeout_get(net.net_idx, net.dst, lpn_address, &poll_timeout);
	if (err) {
...
static int cmd_net_transmit(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc < 2) {
		...
		err = bt_mesh_cfg_net_transmit_set(net.net_idx, net.dst, new_transmit, &transmit);
	}
...
static int cmd_net_key_add(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (IS_ENABLED(CONFIG_BT_MESH_CDB)) {
		...
		if (subnet) {
			if (has_key_val) {
				shell_error(sh, "Subnet 0x%03x already has a value",
					    key_net_idx);
				return 0;
			}
...
static int cmd_net_key_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	shell_print(sh, "NetKeys known by 0x%04x:", net.dst);
	for (i = 0; i < cnt; i++) {
...
static int cmd_app_key_add(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (IS_ENABLED(CONFIG_BT_MESH_CDB)) {
		...
		if (app_key) {
			if (has_key_val) {
				shell_error(sh, "App key 0x%03x already has a value",
					    key_app_idx);
				return 0;
			}
	...
	err = bt_mesh_cfg_app_key_add(net.net_idx, net.dst, key_net_idx, key_app_idx, key_val,
				      &status);
	if (err) {
	...
	if (status) {
		...
		shell_print(sh, "AppKey added, NetKeyIndex 0x%04x AppKeyIndex 0x%04x",
...
static int cmd_app_key_upd(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_app_key_update(net.net_idx, net.dst, key_net_idx, key_app_idx, key_val,
					 &status);
	if (err) {
...
static int cmd_app_key_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_app_key_get(net.net_idx, net.dst, net_idx, &status, keys, &cnt);
	if (err) {
	...
	shell_print(sh, "AppKeys for NetKey 0x%03x known by 0x%04x:", net_idx, net.dst);
	for (i = 0; i < cnt; i++) {
		shell_print(sh, "\t0x%03x", keys[i]);
	}
...
static int cmd_node_id(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc <= 2) {
		printk("ANders\n");
		err = bt_mesh_cfg_node_identity_get(net.net_idx, net.dst, net_idx, &status,
						    &identify);
		if (err) {
...
static int cmd_mod_app_bind(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_app_bind_vnd(net.net_idx, net.dst, elem_addr, mod_app_idx,
						   mod_id, cid, &status);
	} else {
		err = bt_mesh_cfg_mod_app_bind(net.net_idx, net.dst, elem_addr, mod_app_idx, mod_id,
					       &status);
	}
...
static int cmd_mod_app_unbind(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_app_unbind_vnd(net.net_idx, net.dst, elem_addr, mod_app_idx,
						     mod_id, cid, &status);
	} else {
		err = bt_mesh_cfg_mod_app_unbind(net.net_idx, net.dst, elem_addr, mod_app_idx,
						 mod_id, &status);
	}
...
static int cmd_mod_app_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 3) {
		...
		err = bt_mesh_cfg_mod_app_get_vnd(net.net_idx, net.dst, elem_addr, mod_id, cid,
						  &status, apps, &cnt);
	} else {
	...
	if (status) {
		...
		shell_print(sh, "Apps bound to Element 0x%04x, Model 0x%04x %s:", elem_addr,
...
static int cmd_mod_sub_add(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_sub_add_vnd(net.net_idx, net.dst, elem_addr, sub_addr, mod_id,
						  cid, &status);
	} else {
		err = bt_mesh_cfg_mod_sub_add(net.net_idx, net.dst, elem_addr, sub_addr, mod_id,
					      &status);
	}
...
static int cmd_mod_sub_del(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_sub_del_vnd(net.net_idx, net.dst, elem_addr, sub_addr, mod_id,
						  cid, &status);
	} else {
		err = bt_mesh_cfg_mod_sub_del(net.net_idx, net.dst, elem_addr, sub_addr, mod_id,
					      &status);
	}
...
static int cmd_mod_sub_add_va(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_sub_va_add_vnd(net.net_idx, net.dst, elem_addr, label, mod_id,
						     cid, &sub_addr, &status);
	} else {
		err = bt_mesh_cfg_mod_sub_va_add(net.net_idx, net.dst, elem_addr, label, mod_id,
						 &sub_addr, &status);
	}
...
static int cmd_mod_sub_del_va(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_sub_va_del_vnd(net.net_idx, net.dst, elem_addr, label, mod_id,
						     cid, &sub_addr, &status);
	} else {
		err = bt_mesh_cfg_mod_sub_va_del(net.net_idx, net.dst, elem_addr, label, mod_id,
						 &sub_addr, &status);
	}
...
static int cmd_mod_sub_ow(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_sub_overwrite_vnd(net.net_idx, net.dst, elem_addr, sub_addr,
							mod_id, cid, &status);
	} else {
		err = bt_mesh_cfg_mod_sub_overwrite(net.net_idx, net.dst, elem_addr, sub_addr,
						    mod_id, &status);
	}
...
static int cmd_mod_sub_ow_va(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_sub_va_overwrite_vnd(net.net_idx, net.dst, elem_addr, label,
							   mod_id, cid, &sub_addr, &status);
	} else {
		err = bt_mesh_cfg_mod_sub_va_overwrite(net.net_idx, net.dst, elem_addr, label,
...
static int cmd_mod_sub_del_all(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 3) {
		...
		err = bt_mesh_cfg_mod_sub_del_all_vnd(net.net_idx, net.dst, elem_addr, mod_id, cid,
						      &status);
	} else {
		err = bt_mesh_cfg_mod_sub_del_all(net.net_idx, net.dst, elem_addr, mod_id, &status);
	}
...
static int cmd_mod_sub_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 3) {
		...
		err = bt_mesh_cfg_mod_sub_get_vnd(net.net_idx, net.dst, elem_addr, mod_id, cid,
						  &status, subs, &cnt);
	} else {
		err = bt_mesh_cfg_mod_sub_get(net.net_idx, net.dst, elem_addr, mod_id, &status,
					      subs, &cnt);
	}
...
static int cmd_krp(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc < 3) {
		err = bt_mesh_cfg_krp_get(net.net_idx, net.dst, key_net_idx, &status, &phase);
	} else {
		...
		err = bt_mesh_cfg_krp_set(net.net_idx, net.dst, key_net_idx, trans, &status,
					  &phase);
	}

	if (err) {
		shell_error(sh, "Unable to send krp Get/Set (err %d)", err);
		return 0;
	}

	shell_print(sh, "Krp Get/Set with status 0x%02x and phase 0x%02x", status, phase);
...
static int mod_pub_get(const struct shell *sh, uint16_t addr, uint16_t mod_id, uint16_t cid)
{
	...
	if (cid == CID_NVAL) {
		...
		err = bt_mesh_cfg_mod_pub_get_vnd(net.net_idx, net.dst, addr, mod_id, cid, &pub,
						  &status);
	}
		       ...
		       uint16_t cid, char *argv[])
{
	...
	if (cid == CID_NVAL) {
		...
		err = bt_mesh_cfg_mod_pub_set_vnd(net.net_idx, net.dst, addr, mod_id, cid, &pub,
						  &status);
	}
...
static int hb_pub_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	shell_print(sh, "\tdst 0x%04x count 0x%02x period 0x%02x", pub.dst, pub.count,
		    pub.period);
	shell_print(sh, "\tttl 0x%02x feat 0x%04x net_idx 0x%04x", pub.ttl, pub.feat,
		    pub.net_idx);
...
static int cmd_target_set(const struct shell *sh, size_t argc, char *argv[])
{
	int err = 0;
	uint16_t dst = shell_strtoul(argv[1], 0, &err);
	uint16_t net_idx = shell_strtoul(argv[2], 0, &err);
	...
	shell_print(sh,
		    "Cfg client is currently targeting config server @ addr: 0x%04x, net id %d",
		    net.dst, net.net_idx);
	return err;
}

static int cmd_target_get(const struct shell *sh, size_t argc, char *argv[])
{
	shell_print(sh,
		    "Cfg client is currently targeting config server @ addr: 0x%04x, net id %d",
		    net.dst, net.net_idx);
...
{
	shell_print(sh, "This shell module grants control over the config client model "
			   "on a device.\nBefore using any config client command, the target "
			   "config server must be set through the \"Target\" sub-command menu.");

	return 0;
}

SHELL_STATIC_SUBCMD_SET_CREATE(target_cmds,
	SHELL_CMD_ARG(set, NULL, "<Dst addr> <NetKeyIndex>", cmd_target_set, 3, 0),
	SHELL_CMD_ARG(get, NULL, NULL, cmd_target_get, 1, 0),
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(hb-pub, NULL, "[<Dst> <Count> <Period> <TTL> <Features> <NetKeyIndex>]",
		      cmd_hb_pub, 1, 6),
	SHELL_CMD(appkey, &appkey_cmds, "Appkey config commands", shell_model_cmds_help),
	SHELL_CMD(netkey, &netkey_cmds, "Netkey config commands", shell_model_cmds_help),
	SHELL_CMD(model, &model_cmds, "Model config commands", shell_model_cmds_help),
	SHELL_CMD(target, &target_cmds, "Target commands", shell_model_cmds_help),
