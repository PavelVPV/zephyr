...
int bt_mesh_shl_mdl_instance_set(const struct shell *sh, struct bt_mesh_model **mod,
...
int bt_mesh_shl_mdl_print_all(const struct shell *sh, uint16_t mod_id)
{
...
int bt_mesh_shl_mdl_cmds_help(const struct shell *sh, size_t argc, char **argv)
{
	if (argc == 1) {
		shell_help(sh);
		return 0;
