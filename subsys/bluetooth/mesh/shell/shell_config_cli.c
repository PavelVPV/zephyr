...
static int cmd_reset(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_node_reset(bt_mesh_shell_target_ctx.net_idx, bt_mesh_shell_target_ctx.dst,
				     &reset);
	if (err) {
...
static int cmd_get_comp(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_comp_data_get(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_beacon(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc < 2) {
		err = bt_mesh_cfg_beacon_get(bt_mesh_shell_target_ctx.net_idx,
		...
		err = bt_mesh_cfg_beacon_set(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_ttl(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc < 2) {
		err = bt_mesh_cfg_ttl_get(bt_mesh_shell_target_ctx.net_idx,
		...
		err = bt_mesh_cfg_ttl_set(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_friend(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc < 2) {
		err = bt_mesh_cfg_friend_get(bt_mesh_shell_target_ctx.net_idx,
		...
		err = bt_mesh_cfg_friend_set(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_gatt_proxy(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc < 2) {
		err = bt_mesh_cfg_gatt_proxy_get(bt_mesh_shell_target_ctx.net_idx,
		...
		err = bt_mesh_cfg_gatt_proxy_set(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_polltimeout_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_lpn_timeout_get(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_net_transmit(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc < 2) {
		err = bt_mesh_cfg_net_transmit_get(bt_mesh_shell_target_ctx.net_idx,
						   bt_mesh_shell_target_ctx.dst, &transmit);
	} else {
		if (argc != 3) {
			shell_error(sh, "Wrong number of input arguments"
		...
		err = bt_mesh_cfg_net_transmit_set(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_relay(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc < 2) {
		err = bt_mesh_cfg_relay_get(bt_mesh_shell_target_ctx.net_idx,
		...
		err = bt_mesh_cfg_relay_set(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_net_key_add(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_net_key_add(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_net_key_update(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_net_key_update(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_net_key_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_net_key_get(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_net_key_del(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_net_key_del(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_app_key_add(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_app_key_add(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_app_key_upd(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_app_key_update(bt_mesh_shell_target_ctx.net_idx,
					 bt_mesh_shell_target_ctx.dst, key_net_idx, key_app_idx,
					 key_val, &status);
	if (err) {
...
static int cmd_app_key_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_app_key_get(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_node_id(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc <= 2) {
		printk("ANders\n");
		err = bt_mesh_cfg_node_identity_get(bt_mesh_shell_target_ctx.net_idx,
		...
		err = bt_mesh_cfg_node_identity_set(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_app_key_del(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_app_key_del(bt_mesh_shell_target_ctx.net_idx,
				      bt_mesh_shell_target_ctx.dst, key_net_idx, key_app_idx,
				      &status);
	if (err) {
...
static int cmd_mod_app_bind(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_app_bind_vnd(bt_mesh_shell_target_ctx.net_idx,
						   bt_mesh_shell_target_ctx.dst, elem_addr,
						   mod_app_idx, mod_id, cid, &status);
	} else {
		err = bt_mesh_cfg_mod_app_bind(bt_mesh_shell_target_ctx.net_idx,
...
static int cmd_mod_app_unbind(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_app_unbind_vnd(bt_mesh_shell_target_ctx.net_idx,
						     bt_mesh_shell_target_ctx.dst, elem_addr,
						     mod_app_idx, mod_id, cid, &status);
	} else {
		err = bt_mesh_cfg_mod_app_unbind(bt_mesh_shell_target_ctx.net_idx,
						 bt_mesh_shell_target_ctx.dst, elem_addr,
...
static int cmd_mod_app_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 3) {
		...
		err = bt_mesh_cfg_mod_app_get_vnd(bt_mesh_shell_target_ctx.net_idx,
						  bt_mesh_shell_target_ctx.dst, elem_addr, mod_id,
						  cid, &status, apps, &cnt);
	} else {
		err = bt_mesh_cfg_mod_app_get(bt_mesh_shell_target_ctx.net_idx,
					      bt_mesh_shell_target_ctx.dst, elem_addr, mod_id,
					      &status, apps, &cnt);
	}
...
static int cmd_mod_sub_add(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_sub_add_vnd(bt_mesh_shell_target_ctx.net_idx,
						  bt_mesh_shell_target_ctx.dst, elem_addr, sub_addr,
						  mod_id, cid, &status);
	} else {
		err = bt_mesh_cfg_mod_sub_add(bt_mesh_shell_target_ctx.net_idx,
					      bt_mesh_shell_target_ctx.dst, elem_addr, sub_addr,
					      mod_id, &status);
	}
...
static int cmd_mod_sub_del(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_sub_del_vnd(bt_mesh_shell_target_ctx.net_idx,
						  bt_mesh_shell_target_ctx.dst, elem_addr, sub_addr,
						  mod_id, cid, &status);
	} else {
		err = bt_mesh_cfg_mod_sub_del(bt_mesh_shell_target_ctx.net_idx,
					      bt_mesh_shell_target_ctx.dst, elem_addr, sub_addr,
					      mod_id, &status);
	}
...
static int cmd_mod_sub_add_va(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_sub_va_add_vnd(bt_mesh_shell_target_ctx.net_idx,
						     bt_mesh_shell_target_ctx.dst, elem_addr, label,
						     mod_id, cid, &sub_addr, &status);
	} else {
		err = bt_mesh_cfg_mod_sub_va_add(bt_mesh_shell_target_ctx.net_idx,
						 bt_mesh_shell_target_ctx.dst, elem_addr, label,
...
static int cmd_mod_sub_del_va(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_sub_va_del_vnd(bt_mesh_shell_target_ctx.net_idx,
						     bt_mesh_shell_target_ctx.dst, elem_addr, label,
						     mod_id, cid, &sub_addr, &status);
	} else {
		err = bt_mesh_cfg_mod_sub_va_del(bt_mesh_shell_target_ctx.net_idx,
						 bt_mesh_shell_target_ctx.dst, elem_addr, label,
						 mod_id, &sub_addr, &status);
	}
...
static int cmd_mod_sub_ow(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_sub_overwrite_vnd(bt_mesh_shell_target_ctx.net_idx,
							bt_mesh_shell_target_ctx.dst, elem_addr,
							sub_addr, mod_id, cid, &status);
	} else {
		err = bt_mesh_cfg_mod_sub_overwrite(bt_mesh_shell_target_ctx.net_idx,
						    bt_mesh_shell_target_ctx.dst, elem_addr,
...
static int cmd_mod_sub_ow_va(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 4) {
		...
		err = bt_mesh_cfg_mod_sub_va_overwrite_vnd(bt_mesh_shell_target_ctx.net_idx,
							   bt_mesh_shell_target_ctx.dst, elem_addr,
							   label, mod_id, cid, &sub_addr, &status);
	} else {
		err = bt_mesh_cfg_mod_sub_va_overwrite(bt_mesh_shell_target_ctx.net_idx,
						       bt_mesh_shell_target_ctx.dst, elem_addr,
...
static int cmd_mod_sub_del_all(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 3) {
		...
		err = bt_mesh_cfg_mod_sub_del_all_vnd(bt_mesh_shell_target_ctx.net_idx,
						      bt_mesh_shell_target_ctx.dst, elem_addr,
						      mod_id, cid, &status);
	} else {
		err = bt_mesh_cfg_mod_sub_del_all(bt_mesh_shell_target_ctx.net_idx,
						  bt_mesh_shell_target_ctx.dst, elem_addr, mod_id,
						  &status);
	}
...
static int cmd_mod_sub_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 3) {
		...
		err = bt_mesh_cfg_mod_sub_get_vnd(bt_mesh_shell_target_ctx.net_idx,
						  bt_mesh_shell_target_ctx.dst, elem_addr, mod_id,
						  cid, &status, subs, &cnt);
	} else {
		err = bt_mesh_cfg_mod_sub_get(bt_mesh_shell_target_ctx.net_idx,
					      bt_mesh_shell_target_ctx.dst, elem_addr, mod_id,
					      &status, subs, &cnt);
	}
...
static int cmd_krp(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc < 3) {
		err = bt_mesh_cfg_krp_get(bt_mesh_shell_target_ctx.net_idx,
					  bt_mesh_shell_target_ctx.dst, key_net_idx, &status,
					  &phase);
	} else {
		...
		err = bt_mesh_cfg_krp_set(bt_mesh_shell_target_ctx.net_idx,
...
static int mod_pub_get(const struct shell *sh, uint16_t addr, uint16_t mod_id, uint16_t cid)
{
	...
	if (cid == CID_NVAL) {
		err = bt_mesh_cfg_mod_pub_get(bt_mesh_shell_target_ctx.net_idx,
					      bt_mesh_shell_target_ctx.dst, addr, mod_id, &pub,
					      &status);
	} else {
		err = bt_mesh_cfg_mod_pub_get_vnd(bt_mesh_shell_target_ctx.net_idx,
		       ...
		       uint16_t cid, char *argv[])
{
	...
	if (cid == CID_NVAL) {
		err = bt_mesh_cfg_mod_pub_set(bt_mesh_shell_target_ctx.net_idx,
					      bt_mesh_shell_target_ctx.dst, addr, mod_id, &pub,
					      &status);
	} else {
		err = bt_mesh_cfg_mod_pub_set_vnd(bt_mesh_shell_target_ctx.net_idx,
						  bt_mesh_shell_target_ctx.dst, addr, mod_id, cid,
						  &pub, &status);
	}
...
static void hb_sub_print(const struct shell *sh, struct bt_mesh_cfg_hb_sub *sub)
{
	shell_print(sh,
...
static int hb_sub_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_hb_sub_get(bt_mesh_shell_target_ctx.net_idx, bt_mesh_shell_target_ctx.dst,
				     &sub, &status);
	if (err) {
...
static int hb_sub_set(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_hb_sub_set(bt_mesh_shell_target_ctx.net_idx, bt_mesh_shell_target_ctx.dst,
				     &sub, &status);
	if (err) {
...
static int hb_pub_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_hb_pub_get(bt_mesh_shell_target_ctx.net_idx, bt_mesh_shell_target_ctx.dst,
				     &pub, &status);
	if (err) {
...
static int hb_pub_set(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_cfg_hb_pub_set(bt_mesh_shell_target_ctx.net_idx, bt_mesh_shell_target_ctx.dst,
				     &pub, &status);
	if (err) {
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(hb-pub, NULL, "[<Dst> <Count> <Period> <TTL> <Features> <NetKeyIndex>]",
		      cmd_hb_pub, 1, 6),
	SHELL_CMD(appkey, &appkey_cmds, "Appkey config commands", bt_mesh_shl_mdl_cmds_help),
	SHELL_CMD(netkey, &netkey_cmds, "Netkey config commands", bt_mesh_shl_mdl_cmds_help),
	SHELL_CMD(model, &model_cmds, "Model config commands", bt_mesh_shl_mdl_cmds_help),
	SHELL_SUBCMD_SET_END);

SHELL_SUBCMD_ADD((mesh, models), cfg, &cfg_cli_cmds, "Config Cli commands",
		 bt_mesh_shl_mdl_cmds_help, 1, 1);
