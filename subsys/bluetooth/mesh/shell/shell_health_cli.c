...
{
	if (!mod && !bt_mesh_shl_mdl_first_get(BT_MESH_MODEL_ID_HEALTH_CLI, &mod)) {
		return -ENODEV;
	}
...
{
	return bt_mesh_shl_mdl_print_all(sh, BT_MESH_MODEL_ID_HEALTH_CLI);
}
...
static int cmd_instance_set(const struct shell *sh, size_t argc, char *argv[])
{
	...
	return bt_mesh_shl_mdl_instance_set(sh, &mod, BT_MESH_MODEL_ID_HEALTH_CLI, elem_idx);
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD(instance, &instance_cmds, "Instance commands", bt_mesh_shl_mdl_cmds_help),
	SHELL_SUBCMD_SET_END);
