...
{
	if (!mod && !shell_model_first_get(BT_MESH_MODEL_ID_HEALTH_CLI, &mod)) {
		return -ENODEV;
	}
	...
	err = bt_mesh_health_cli_fault_get(cli, NULL, cid, &test_id, faults, &fault_count);
	if (err) {
...
static int fault_clear(const struct shell *sh, size_t argc, char *argv[], bool acked)
{
	...
	if (acked) {
		...
		err = bt_mesh_health_cli_fault_clear(cli, NULL, cid, &test_id, faults,
						     &fault_count);
		if (err) {
	...
	err = bt_mesh_health_cli_fault_clear_unack(cli, NULL, cid);
	if (err) {
...
static int fault_test(const struct shell *sh, size_t argc, char *argv[], bool acked)
{
	...
	if (acked) {
		...
		err = bt_mesh_health_cli_fault_test(cli, NULL, cid, test_id, faults, &fault_count);
		if (err) {
	...
	err = bt_mesh_health_cli_fault_test_unack(cli, NULL, cid, test_id);
	if (err) {
...
static int cmd_period_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_health_cli_period_get(cli, NULL, &divisor);
	if (err) {
...
static int period_set(const struct shell *sh, size_t argc, char *argv[], bool acked)
{
	...
	if (acked) {
		...
		err = bt_mesh_health_cli_period_set(cli, NULL, divisor, &updated_divisor);
		if (err) {
	...
	err = bt_mesh_health_cli_period_set_unack(cli, NULL, divisor);
	if (err) {
...
static int cmd_attention_get(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_mesh_health_cli_attention_get(cli, NULL, &attention);
	if (err) {
...
static int attention_set(const struct shell *sh, size_t argc, char *argv[], bool acked)
{
	...
	if (acked) {
		...
		err = bt_mesh_health_cli_attention_set(cli, NULL, attention, &updated_attention);
		if (err) {
	...
	err = bt_mesh_health_cli_attention_set_unack(cli, NULL, attention);
	if (err) {
		shell_error(sh, "Failed to send Health Attention Set (err % d) ", err);
	}
...
{
	return shell_model_instances_get_all(sh, BT_MESH_MODEL_ID_HEALTH_CLI);
}
...
static int cmd_instance_set(const struct shell *sh, size_t argc, char *argv[])
{
	...
	return shell_model_instance_set(sh, &mod, BT_MESH_MODEL_ID_HEALTH_CLI, elem_idx);
}
...
SHELL_CMD_ARG_REGISTER(mdl_health, &health_cli_cmds, "Health Cli commands", shell_model_cmds_help,
		       1, 1);
