...
int shell_model_instance_set(const struct shell *sh, struct bt_mesh_model **mod, uint16_t mod_id,
			     uint8_t elem_idx)
{
...
int shell_model_instances_get_all(const struct shell *sh, uint16_t mod_id)
{
...
int shell_model_cmds_help(const struct shell *sh, size_t argc, char **argv)
{
	if (argc == 1) {
		shell_help(sh);
		return 1;
