...
The simplest way to provision the device is through self-provisioning. To do this the user must create a Configuration Database and provision the device with the default network key and address ``0x0001``, execute::
...
To allow a device to broadcast provisioning beacons, the :kconfig:option:`CONFIG_BT_MESH_PROV_DEVICE` configuration option must be enabled, along with either :kconfig:option:`CONFIG_BT_MESH_PB_GATT` or :kconfig:option:`CONFIG_BT_MESH_PB_ADV`.

To allow a device to provision devices, either the :kconfig:option:`CONFIG_BT_MESH_PROVISIONER` or :kconfig:option:`CONFIG_BT_MESH_PB_GATT_CLIENT` configuration option must be enabled.
...
* Provision other devices (``mesh prov beacon-listen``) (``mesh prov pb-adv``) (``mesh prov pb-gatt``).
...
``mesh models cfg target set <Dst_addr> <NetKeyIndex>``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
``mesh models health instance get_all``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
