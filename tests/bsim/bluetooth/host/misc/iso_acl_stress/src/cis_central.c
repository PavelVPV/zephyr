...
static struct k_work_q iso_send_work_q;
static K_THREAD_STACK_DEFINE(iso_send_work_stack, 1024);
...
static void iso_send_work_handler(struct k_work *work)
{
	...
	if (ret < 0) {
		...
		k_work_reschedule_for_queue(&iso_send_work_q, k_work_delayable_from_work(work),
					    K_USEC(interval_us));
...
static void start_iso_send(void)
{
	k_work_queue_start(&iso_send_work_q, iso_send_work_stack,
			   K_THREAD_STACK_SIZEOF(iso_send_work_stack),
			   K_PRIO_COOP(1), NULL);
...
static void sdu_sent_cb(struct bt_iso_chan *chan)
{
	...
	err = k_work_schedule_for_queue(&iso_send_work_q, &iso_send_work, K_NO_WAIT);
	if (err < 0) {
			     ...
			     struct bt_gatt_discover_params *params)
{
	...
	if (attr == NULL) {
		if (test_chrc_handle == 0) {
			TEST_FAIL("Did not discover long_chrc (%x)", test_chrc_handle);
		}
	...
	if (params->type == BT_GATT_DISCOVER_PRIMARY &&
		...
		if (bt_uuid_cmp(chrc->uuid, TEST_CHRC_UUID) == 0) {
			printk("Found test_chrc\n");
			test_chrc_handle = chrc->value_handle;
		}
	}
...
static struct k_work_q acl_send_work_q;
static K_THREAD_STACK_DEFINE(acl_send_work_stack, 1024);

static void acl_send_work_handler(struct k_work *work);
K_WORK_DEFINE(acl_send_work, acl_send_work_handler);

K_SEM_DEFINE(gatt_write_sem, CONFIG_BT_ATT_TX_COUNT, CONFIG_BT_ATT_TX_COUNT);

static void gatt_write_without_rsp_complete(struct bt_conn *conn, void *user_data)
{
...
static void gatt_write_without_rsp(void)
{
	uint8_t data[10] = {};
	int err;

	err = bt_gatt_write_without_response_cb(default_conn, test_chrc_handle, data, sizeof(data),
						false, gatt_write_without_rsp_complete, NULL);
	if (err != 0) {
		TEST_FAIL("Write without response failed (err %d)", err);
	}
}
...
static void acl_send_work_handler(struct k_work *work)
{
	...
	/* We need to fill up ATT TX pool before Host lets TX process to send ACL data towards
	 * Controller. Therefore, first schedule commands, and only then wait for completion.
...
static void start_acl_send(void)
{
	k_work_queue_start(&acl_send_work_q, acl_send_work_stack,
			   K_THREAD_STACK_SIZEOF(acl_send_work_stack),
			   K_PRIO_COOP(1), NULL);
