...
static void connected_deadlock_cb(struct bt_conn *conn, uint8_t err)
{
	TEST_ASSERT(conn);
	TEST_ASSERT(err == BT_HCI_ERR_SUCCESS, "Expected connection establishment");

#if 0
	BUILD_ASSERT(!IS_ENABLED(CONFIG_BT_AUTO_DATA_LEN_UPDATE));
	BUILD_ASSERT(IS_ENABLED(CONFIG_BT_AUTO_PHY_CENTRAL_NONE));

	err = bt_hci_le_read_remote_features(conn);
	TEST_ASSERT(err == 0, "Failed read remote features (err %d)", err);

	err = bt_hci_read_remote_version(conn);
	TEST_ASSERT(err == 0, "Failed read remote version (err %d)", err);

	err = bt_le_set_phy(conn, 0U, BT_HCI_LE_PHY_1M, BT_HCI_LE_PHY_PREFER_1M, BT_HCI_LE_PHY_CODED_ANY);
	TEST_ASSERT(err == 0, "Failed set PHY (err %d)", err);

	uint16_t tx_octets, tx_time;

	err = bt_hci_le_read_max_data_len(&tx_octets, &tx_time);
	TEST_ASSERT(err == 0, "Failed read max data len (err %d)", err);

	err = bt_le_set_data_len(conn, tx_octets, tx_time);
	TEST_ASSERT(err == 0, "Failed set data len (err %d)", err);
#endif
#if 0
	k_sleep(K_MSEC(1));
	...
	TEST_ASSERT(err == 0, "Failed update conn params (err %d)", err);
#endif
	k_sleep(K_MSEC(1000));
...
static struct bt_conn_cb conn_deadlock_cb = {
	.connected = connected_deadlock_cb,
};

static void test_central_deadlock(void)
{
	int err;

	bt_conn_cb_register(&conn_deadlock_cb);
...
static const struct bst_test_instance test_def[] = {
	...
	{
		.test_id = "central_deadlock",
		.test_descr = "TBU",
		.test_main_f = test_central_deadlock,
	},
